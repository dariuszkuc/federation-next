...
use crate::query_graph::graph_path::{selection_of_element, OpGraphPathContext, OpGraphPathTrigger, OpPath, OpPathElement, concat_paths_in_parents};
...
use crate::schema::position::{CompositeTypeDefinitionPosition, FieldDefinitionPosition, ObjectTypeDefinitionPosition, OutputTypeDefinitionPosition, SchemaRootDefinitionKind};
...
use apollo_compiler::executable::{self, Operation, VariableDefinition};
...
impl FetchDependencyGraph {
    ...
    fn is_parent_of(&self, node_id: NodeIndex, child_id: NodeIndex) -> bool {
        self.parents_of(node_id).any(|id| id == child_id)
    }
    ...
    fn remove_node(&mut self, node_index: NodeIndex) {
        let edges_to_remove = self.graph.edges(node_index);
    ...
    fn can_merge_sibling_in(&self, node_id: NodeIndex, sibling_id: NodeIndex) -> Result<bool, FederationError> {
        ...
        // - ensuring the same "merge_at" makes so we can merge the inputs and selections without having to worry about those
        ...
        let mut own_parents = self.parents_relations_of(node_id);
        let own_parent = own_parents.next();

        let sibling = self.node_weight(sibling_id)?;
        let mut sibling_parents = self.parents_relations_of(sibling_id);
        let sibling_parent = sibling_parents.next();
        Ok(node.defer_ref == sibling.defer_ref
            && node.subgraph_name == sibling.subgraph_name
            && node.merge_at == sibling.merge_at
            && own_parent.is_some()
            && own_parents.next().is_none()
            && sibling_parent.is_some()
            && sibling_parents.next().is_none()
            && own_parent.unwrap().parent_node_id == sibling_parent.unwrap().parent_node_id)
    }

    fn merge_sibling_in(&mut self, node_id: NodeIndex, sibling_id: NodeIndex) -> Result<(), FederationError> {
        let node = FetchDependencyGraph::node_weight_mut(&mut self.graph, node_id)?;
        let sibling = FetchDependencyGraph::node_weight(&self, sibling_id)?;
        node.copy_inputs(&self.supergraph_schema, sibling)?;
        self.merge_in_internal(node_id, sibling_id, &OpPath::default(), false)?;

        Ok(())
    }

    fn merge_in_internal(&mut self, node_id: NodeIndex, merged_id: NodeIndex, path: &OpPath, merge_parent_dependencies: bool) -> Result<(), FederationError> {
        let node = FetchDependencyGraph::node_weight_mut(&mut self.graph, node_id)?;
        let merged = FetchDependencyGraph::node_weight(&self, merged_id)?;

        if !node.is_top_level() {
            return Err(FederationError::internal("Shouldn't remove top level nodes"));
        }

        if path.is_empty() {
            node.selection_set.add_selections(&merged.selection_set.selection_set)?;
        } else {
            ...
            let merge_path_conditional_directives = Conditions::from_directives(&path.conditional_directives())?;
            let merged_selection_set = remove_conditions_from_selection_set(&*merged.selection_set.selection_set, &merge_path_conditional_directives)?;
            node.selection_set.add_at_path(path, Some(&Arc::new(merged_selection_set)))?;
        }

        self.on_modification();
        self.relocate_children_on_merged_in(node_id, merged_id, path);
        if merge_parent_dependencies {
            self.relocate_parents_on_merged_in(node_id, merged_id, path);
        }

        if merged.must_preserve_selection_set {
            node.must_preserve_selection_set = true;
        }
        self.remove_node(merged_id);
        Ok(())
    }

    fn relocate_children_on_merged_in(&mut self, node_id: NodeIndex, merged_id: NodeIndex, path_in_this: &OpPath) {
        for child_id in self.children_of(merged_id) {
            ...
            let path_in_merged = self.parents_relations_of(merged_id)
                .next()
                .map(|p| p.path_in_parent)
                .flatten();
            let concatenated_paths = concat_paths_in_parents(&Some(Arc::new(path_in_this.clone())), &path_in_merged);
            self.add_parent(child_id, ParentRelation { parent_node_id: node_id, path_in_parent: concatenated_paths });
        }
    }

    fn relocate_parents_on_merged_in(&mut self, node_id: NodeIndex, merged_id: NodeIndex, path_in_this: &OpPath) {
        for parent in self.parents_relations_of(merged_id) {
            ...
            self.add_parent(node_id, parent.clone());
        }
    }

    fn type_at_path(&self, node_id: NodeIndex, path: OpPath) -> CompositeTypeDefinitionPosition {
        todo!()
    }
}
...
impl FetchDependencyGraphNode {
    ...
    fn add_inputs(
        ...
        let inputs = self.inputs.get_or_insert_with(|| {
            Arc::new(FetchInputs::empty(supergraph_schema.clone()))
        });
    ...
    fn copy_inputs(&mut self, supergraph_schema: &ValidFederationSchema, other: &FetchDependencyGraphNode) -> Result<(), FederationError> {
...
impl FetchSelectionSet {
    ...
    fn add_selections(
        ...
        Arc::make_mut(&mut self.selection_set).merge_into(std::iter::once(selection_set.deref()))?;
        Ok(())
    }
}
...
fn handle_requires(
    ...
    let head = dependency_graph.federated_query_graph.edge_endpoints(query_graph_edge_id)
        .and_then(|(_tail_id, head_id)| dependency_graph.federated_query_graph.node_weight(head_id))?;

    // @requires should be on an entity type, and we only support object types right now
    let QueryGraphNodeType::SchemaType(OutputTypeDefinitionPosition::Object(entity_type_position)) = head.type_.clone() else {
        return Err(FederationError::internal("not an object"));
    };

    // In many case, we can optimize requires by merging the requirement to previously existing nodes. However,
    ...
    let mut parents= dependency_graph.parents_relations_of(fetch_node_id);
    let mut parent = parents.next();
    let is_single_parent = parent.is_some() && parents.next().is_none();
    ...
    // In that case (when the path is only typeCasts), we can put the created nodes directly
    ...
    // to our current node).
    if is_single_parent && paths_has_only_fragments(&fetch_node_path.path_in_node) {
        let parent = parent.unwrap();
        let mut node = dependency_graph.node_weight(fetch_node_id)?;
        ...
        let Some(merge_at) = node.merge_at.clone() else {
            return Err(FederationError::internal("Merge at path is required but was missing"));
        };
        let new_node_id = dependency_graph.new_key_node(&node.subgraph_name, merge_at, node.defer_ref.clone())?;
        ...
        new_node.copy_inputs(&dependency_graph.supergraph_schema, node)?;

        let created_nodes = compute_nodes_for_tree(dependency_graph, requires_conditions, fetch_node_id, fetch_node_path.clone(), defer_context_for_conditions(defer_context), &OpGraphPathContext::default())?;
        if created_nodes.len() == 0 {
            // All conditions were local. Just merge the newly created node back in the current node (we didn't need it)
            // and continue.
            if !dependency_graph.can_merge_sibling_in(fetch_node_id, new_node_id)? {
                return Err(FederationError::internal(format!("We should be able to merge {} into {} by construction", new_node_id.index(), fetch_node_id.index())));
            }
            dependency_graph.merge_sibling_in(fetch_node_id, new_node_id)?;
            return Ok((fetch_node_id, fetch_node_path.clone()));
        }

        // We know the @require needs created_nodes. We do want to know however if any of the conditions was
        // fetched from our `new_node`. If not, then this means that the `created_nodes` don't really depend on
        ...
        new_node.remove_inputs_from_selection();
        let type_at_path = dependency_graph.type_at_path(parent.parent_node_id, parent.path_in_parent);
        let new_node_is_unneeded = parent.path_in_parent.is_some() &&
            new_node.selection_set.selection_set.can_rebase_on(&type_at_path);

        let unmerged_nodes: Vec<NodeIndex> = Vec::new();
    ...
    // PORT_NOTE: instead of returning IDs of created nodes they should be inserted directly
...
fn paths_has_only_fragments(path: &Arc<OpPath>) -> bool {
    ...
    path.0.iter().all(|p| matches!(p.as_ref(), OpPathElement::InlineFragment(_)))
}
